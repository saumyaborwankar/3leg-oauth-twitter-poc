...
export default function SignIn(props) {
  useEffect(() => {
    ...
    if (query.oauth_token === localStorage.getItem("oauthRequestToken")) {
      console.log("oauth token good");
      // makes call to callback endpoint(on our server) with the needed params
      axios
        .get(
          `http://localhost:8080/callback/${localStorage.getItem(
            "oauthRequestToken"
        ...
        .then((response) => {
          if (response.data.oauthAccessToken) {
            ...
            axios
              .get(
                `http://localhost:8080/verify/${response.data.oauthAccessToken}/${response.data.oauthAccessTokenSecret}`
              )
              .then((res) => {
                const { user } = res.data;
        ...
        .catch((err) => {
          alert("r authentication error");
          // props.history.push({
          //   pathname: "/",
          // });
        });
    } else {
      alert("s authentication error");
